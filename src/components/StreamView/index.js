...
class StreamView extends React.Component {
  constructor(props) {
    ...
    if (params.subreddit || !params.user) {
      this.initForSubreddit(params.subreddit)
    } else if (params.user) {
      this.initForUser(params.user)
    }
  ...
  initForUser(user) {
    ...
    state.user = user
    state.href =
      window.location.href.search(/\/(u|user)\//) >= 0
        ? '/user/' + state.user
        : '/'
  ...
  initForSubreddit(subreddit) {
    ...
    state.href =
      window.location.href.indexOf('/r/') >= 0 ? '/r/' + state.subreddit : '/'
    state.text =
      state.subreddit == this.defaultSubreddit
        ? 'Frontpage'
        : '/r/' + state.subreddit
  }
  ...
  loadUser(user = this.state.user, options = { reset: false }) {
    ...
    this.setState(state, () => {
      ...
      reddit.getPostsFromUser(user, options, (err, posts) => {
        ...
        this.setState({
          postViews: this.createViewsFromRedditPosts(
            newPosts,
  ...
  didStopScrolling() {
    ...
    for (var i = 0; i < postNodes.length; i++) {
      ...
      if (
        post.offsetTop + post.clientHeight < startY ||
        post.offsetTop > endY
      ) {
  ...
  render() {
    window.postViews = this.state.postViews
    var loading = this.state.isLoading ? <StreamSpinnerView /> : false
    var notFound = this.state.notFound ? (
      <div>Subreddit {this.state.subreddit} does not exist.</div>
    ) : (
      false
    )
